...
import { d as displayDate } from './time-f374952b.js';
...
const getNumberAnnotations = (annotations) => {
    ...
    if (numberAnnotations.length < 1) {
        const { y } = annotations, annotationProps = __rest(annotations, ["y"]);
        return annotationProps;
    }
    return Object.assign(Object.assign({}, annotations), { y: numberAnnotations });
};
const valueDisplayText = ({ value, resolution, viewPort, }) => {
...
const getValueText = ({ annotation, resolution, viewPort, }) => {
    const valueText = annotation.showValue ? valueDisplayText({ value: annotation.value, resolution, viewPort }) : null;
    if (valueText) {
        return `${valueText}`;
    }
...
export { getDataPoints as a, getNumberThresholds as b, getThresholds as c, getValueAndText as d, getColor as e, getText as f, getBreachedThreshold as g, getValueText as h, isThreshold as i, getNumberAnnotations as j, sortThreshold as s };
