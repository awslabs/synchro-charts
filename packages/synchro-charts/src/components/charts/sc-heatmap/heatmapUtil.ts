import { DataStream, Primitive, ViewPort, DataPoint} from '../../../utils/dataTypes';
...
const calculateBucketIndex = (yValue: number, yMax: number, bucketCount: number): number => {
  return Math.ceil((yValue / yMax) * bucketCount);
};
 ...
 */
export const addCount = (oldHeatValue: HeatValueMap = {}, xBucketRangeStart: number, bucketIndex: number, dataStreamName:string): HeatValueMap => {
  ...
  newHeatValue[xBucketRangeStart][bucketIndex][dataStreamName] = newHeatValue[xBucketRangeStart][bucketIndex][dataStreamName] ?? 0;
 ...
 * @returns Updated HeatValueMap object with the new data added -- not destructive.
 */
const countFunc = (oldHeatValue: HeatValueMap, dataPoint: DataPoint, xBucketRangeStart: number, yMax: number, dataStreamName: string): HeatValueMap => {
  const { y } = dataPoint;
  const bucketIndex = calculateBucketIndex(parseInt(y.toString()), yMax, bucketCount);
...
export const calcHeatValues = (
  ...
  dataStreams.forEach((dataStream) => {
	...
	return;
      ...
      let currXBucketRangeStart = nextTimeStamp - XAxisBucketRange;
      if (!heatValue[currXBucketRangeStart]) {
        if (newHeatValue) {
	  newHeatValue = countFunc(newHeatValue, point, currXBucketRangeStart, yMax, dataStream.name);
        } else {
	  newHeatValue = countFunc(heatValue, point, currXBucketRangeStart, yMax, dataStream.name);
        }
      }
    });
  });
