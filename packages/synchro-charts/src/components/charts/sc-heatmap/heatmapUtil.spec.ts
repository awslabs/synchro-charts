...
import { DataPoint, DataStream, Primitive, ViewPort } from '../../../utils/dataTypes'
...
describe('simple calcHeatValue tests', () => {
  it('calcHeatValue has proper structure', () => {
    ...
    const viewPort: ViewPort = {start: new Date('June 30, 2021 10:00:00'),
      end: new Date('June 30, 2022 21:00:00'),
      yMin: 0,
      yMax: 100,
    };
    var startTime = viewPort.start.getTime();
    var endTime = viewPort.end.getTime();
    var yMax = viewPort.yMax;
    testHeatValue = calcHeatValues(heatValue, 
      dataStreams, 
      resolution, 
      startTime,
      endTime,
  ...
  it('calcHeatValues has individual datastream counts', () => {
    ...
    var endTime = viewPort.end.getTime();
    var yMax = viewPort.yMax;
//     const heatmapDatastreamProperties = dataStreams.map(stream => [stream.data, stream.name]);
//     console.log(heatmapDatastreamProperties);
    heatValue = calcHeatValues(heatValue, 
      dataStreams, 
      resolution, 
      startTime,
      endTime,
